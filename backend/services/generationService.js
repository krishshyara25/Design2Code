...
// We use the native 'https' module for robust API requests.
const https = require('https');

/**
 * Sends a prompt to the Google Gemini API and returns the generated code.
 * @param {string} userPrompt - The detailed prompt constructed by our conversation service.
 * @returns {Promise<string|null>} The generated HTML/CSS code or null if an error occurs.
 */
async function getCodeFromAI(userPrompt) {
    // --- FINAL ARCHITECTURE FIX ---
    // The API key is now read inside the function, ensuring it's available when needed.
    const apiKey = process.env.GEMINI_API_KEY;

    // The pre-flight check remains crucial.
    if (!apiKey) {
    ...
    // We now wrap the https request in a Promise for clean async/await usage.
    return new Promise((resolve, reject) => {
                ...
                if (res.statusCode >= 200 && res.statusCode < 300) {
                    try {
                        ...
                        if (responseBody.candidates && responseBody.candidates[0].content.parts[0].text) {
                            ...
                            // Handle cases where the AI response is valid but empty (e.g., safety filters)
        ...
        // Send the payload and finalize the request.
        req.write(payload);
        req.end();
    });
}
